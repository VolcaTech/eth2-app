...
class ConfirmSmsForm extends Component {
	...
	return (
		...
		{ this.state.timer ? 
		    <div style={{ height: 28, width: 210, margin: 'auto', marginBottom: 46, display: this.state.timer ? 'block': 'none', textAlign: 'center', fontSize: 12 }}>Send code again in  seconds <Timer style={{ display: 'inline-block' }} afterComplete={() => this.setState({smsButtonDisabled: false, timer: false})} interval={1000} remaining={60000}>
		    <Countdown />
		  </Timer> 	  
		    </div> : null }
	    </div>
